...
> Note V01c (2026-07-11) : nouveau `components/layout/trip-nav.tsx` — `TripTabBar` (mobile : barre basse fixe h-56+safe-area, 5 items Journal·Bourse·Idées·Plus·Profil, soulignement citron 20×3 sous le label, point berry sans compteur) et `TripSidebar` (desktop ≥ lg : colonne 220 sticky, primaires + rubriques à plat) ; le Journal couvre timeline/map/day/events. `TripTabs` (12 onglets scrollables) supprimé. Nouvelle grille « Plus » (`trips/[tripId]/plus`, 10 rubriques icônes Lucide, urgences incluses). Layout voyage : conteneur max-w-content (1200), header cover + Badge B5 (à venir=citron, en cours=lagune), bandeau passeport en berry-wash/citron-wash, pb-24 mobile pour la barre. Header d'app : wordmark Schibsted 800, nav desktop réduite à Voyages·Coffre (hub Profil : le menu avatar gagne Amis/Horloges/Conseils — la page Profil en rangées B6 arrive en V01o), emplacement cloche réservé (V04). i18n ×3 : `tripTabs.plus/profile/emergency` ajoutés, fr `budget`→« Bourse », `participants`→« Équipage » (en « Crew », es « Tripulación »). Écart assumé vs prototype : Carte reste une sous-vue du Journal (canon §4) et non un onglet de premier niveau.
