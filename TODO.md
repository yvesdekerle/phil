...
- [ ] V07e — Dépenses : **voir la répartition d'une dépense et la modifier** (clic sur la ligne → panneau détail : montant, payeur, bénéficiaires et part de chacun ; éditable par créateur/payeur/capitaine tant que la Bourse est ouverte). Migration : policy UPDATE sur `expenses`, élargissement de l'insert des bénéficiaires au payeur/capitaine, RPC transactionnelle `update_expense_with_beneficiaries` ; `pnpm verify:rls` après.
