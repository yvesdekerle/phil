...
- [ ] V07d — Idées : **numéroter les idées géolocalisées et les relier à la carte** (clic sur le numéro → la carte zoome sur le lieu, pastilles numérotées assorties). Layout desktop façon « Carte » : liste à gauche, carte sticky à droite — les actions des cartes d'idées gardent leur place, la carte reste visible en scrollant. Sa question « même système que la Carte ? » : oui pour le principe liste+carte, mais liste large / carte en colonne (l'inverse de la page Carte) pour préserver la place des actions.
