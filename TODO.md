...
> Note : bouton "Importe-la" sur Nouvel événement → page `/events/import` (OWNER/EDITOR) : dépôt PDF ou 1-5 captures (4 Mo total, limite requête Vercel) → route `POST /api/trips/{id}/import-reservation` (aucune écriture) → **formulaire de relecture pré-rempli** (type, dates locales + fuseau — extrait si IANA valide, sinon celui du voyage —, trajet/lieu, mode, compagnie, référence, notes) → à la confirmation : événement créé + **la confirmation rangée en document TRIP** (catégorie billet/hébergement) attaché à l'événement, géocodage N01. Extraction `lib/import/reservation.ts` : Gemini REST (`GEMINI_MODEL`, défaut gemini-2.5-flash), JSON forcé, Zod tolérant (`catch`), consigne "jamais deviner une date absente → null". Sans `GEMINI_API_KEY` (ajoutée à `.env.example`) : page affiche "non configuré", reste de l'app intact. ~~**À faire côté Yves : créer la clé AI Studio et la poser en local + Vercel, puis test réel avec une vraie confirmation.**~~ *(clé posée + test réel OK le 2026-07-12, cf. O01b)* Écart : plusieurs captures analysées ensemble mais seul le premier fichier est attaché en document.

### [x] PHIL-O01b — Modèle Gemini par défaut obsolète pour les nouvelles clés *(fait le 2026-07-12)*
Découvert en posant la clé d'Yves (projet Google créé le 2026-07-12) : `generateContent` sur `gemini-2.5-flash` répond **404 « no longer available to new users »** pour les clés récentes (le modèle reste listé par ListModels, piège). Testé en réel via `tmp/test-gemini-import.ts` (PDF de confirmation générée par pdf-lib → vrai pipeline `extractReservation`) : `gemini-3.5-flash` et `gemini-flash-latest` **dépassent les 30 s** du timeout du code (free tier congestionné), `gemini-3.1-flash-lite` répond en **1,7 s** avec une extraction parfaite (type, dates locales, fuseau, trajet, compagnie, référence, notes).
> Fix : défaut `GEMINI_MODEL` → `gemini-3.1-flash-lite` (`lib/import/reservation.ts` + commentaire `.env.example`). `GEMINI_MODEL` reste surchargeable par env si besoin.
...
### [x] PHIL-V01 — Épopée : intégrer le design « Lagune vive » v1 (couche présentation uniquement) *(fait le 2026-07-11)*
...
- [x] V01l — Événements : détail / création / édition / import *(fait le 2026-07-11)* — chip vol ink-deep mono « ✈ HH:mm » sur le détail des transports, erreurs de formulaires en berry-ink + caption (9 occurrences héritées du renommage bordeaux→lagoon-ink triées), titres à l échelle typo (title/heading/subhead), badges mono-caps. Les formulaires héritent des primitives B1/B2.
- [x] V01m — Équipage + Réglages + Urgence *(fait le 2026-07-11)* — badge capitaine B5 (ink/citron, unique), rôles en mono-caps, noms en subhead ; erreurs de formulaires (réglages, invitations, urgence, cover) en berry-ink + caption ; titres à l échelle typo. Les zones destructives héritent du Button destructive (berry-wash/berry-ink).
- [x] V01n — Coffre : verrouillé / ouvert *(fait le 2026-07-11)* — nouvel écran de verrou partagé `vault-lock-screen.tsx` (surface radiale ink→ink-deep, cadre de scan lagune pulsant r-32, ligne de balayage citron + « Vérification… » mono clignotant, CTA Déverrouiller pill lagoon-ink + glow) utilisé par VaultUnlock (WebAuthn C05) et CoffreGate (E2EE T01) ; **porte animée v1 supprimée** (vault-door.tsx + webp + page demo-animations). Liste du coffre : noms subhead, méta mono-caps, chips de filtre B4, badges Expiré (berry-wash) / Expire bientôt (citron-wash) en mono. Écarts : bannière « SE VERROUILLE 4:59 » non affichée (expiration de session httpOnly côté serveur — à câbler plus tard si souhaité) ; keyframes ring-pulse/scan-move/dot-blink neutralisées en reduced-motion.
- [x] V01o — Transverse : Profil, Sécurité, Amis, Horloges, Conseils, Explorer *(fait le 2026-07-11)* — le Profil devient le hub du handoff §4 : carte de rangées B6 (Amis · Horloges · Conseils · Exploration · Sécurité) avec chevrons, titres à l échelle typo, zone dangereuse en berry (bordure + lavis + bouton berry-ink), erreurs de formulaires en berry-ink sur les 6 pages.
- [x] V01p — Auth, pages publiques & états système *(fait le 2026-07-11)* — login refait en carte flottante Lagune vive (wordmark Schibsted 800 + kicker mono, le « billet d embarquement » v1 disparaît) ; **PhilLoader supprimé** (montgolfière/globe webp inclus) → squelettes wash r-8 (loading app + loading voyage silhouette du nouveau layout) ; bandeau hors-ligne en berry-wash/berry-ink (canon §6, contenu conservé) ; 404/erreurs/offline/partage public/légal normalisés à l échelle typo ; e2e Playwright 8/8 verts. demo-animations déjà supprimée en V01n.
- [x] V01q — Emails + PWA/manifest + icônes *(fait le 2026-07-11)* — EmailShell : palette déjà migrée en V01a, Georgia → pile système sans-serif, wordmark 800, bouton en pill lagoon-ink ; manifest déjà sand ; **icônes régénérées** (sharp, P géométrique blanc sur encre + soulignement citron — sans dépendance de police) : icon-192/512 + 512-maskable (zone sûre 80 %) + apple-touch + app/icon.png (le favicon.ico v1 disparaît).
- [x] V01r — Phase 4 : nettoyage final + QA *(fait le 2026-07-11)* — grep zéro reliquat confirmé (tokens v1, Bodoni/Figtree, font-display, rgba(31,42,68, #953c32, #ece4d2, montgolfière, porte, phil-loader, vault-door : 0 occurrence) ; 3 derniers hex magiques tokenisés (global-error→slate, marqueur commerces→berry, mer de la carte du monde→sand — seul le logo Google garde ses couleurs de marque) ; CLAUDE.md § « Identité visuelle et ton » réécrit pour Lagune vive (ton Jules Verne conservé) ; build + lint + type-check + 185 tests unitaires + 8 e2e verts. QA visuelle 390/1024 et contrastes à confirmer par Yves au retour (checkpoint reporté à sa demande).
...
- [x] V06b — Mes voyages : la carte sombre « Ton coffre » disparaît en desktop (« que fait ton coffre ici ? » — le header a déjà le lien Coffre) ; conservée en mobile. *(fait le 2026-07-11)*
- [x] V06c — Timeline desktop « vraiment moins jolie qu'avant, jours trop courts » : retour au **Gantt horizontal scrollable** d'avant-refonte (155 px/jour, colonne des noms fixe 220 px, une ligne par événement, en-tête de jours figé) en livrée Lagune vive (colonne d'aujourd'hui citron, vols encre sombre mono, logements lavis lagune). Timeline et Carte s'élargissent à 104 rem via `TripMain` (le gabarit 1200 px les étouffait) ; le ruban mobile L2b est conservé tel quel. *(fait le 2026-07-11)*
...
### [~] PHIL-V07 — Épopée : retours desktop v3 (captures du 2026-07-11)
Deuxième passage d'Yves sur le redesign en desktop (captures timeline/dépenses/voyages passés). Un commit par sous-ticket.
- [x] V07a — **Fix systémique `cn()`** : `tailwind-merge` ne connaît pas les 9 styles de texte du design system (`text-display`…`text-label`) et les classe comme des couleurs — dès qu'une couleur `text-*` suit dans le même `cn()`, la taille est supprimée et le texte retombe sur le 16 px hérité. C'est la cause du déséquilibre signalé dans la timeline (noms 11 px vs barres 16 px). `extendTailwindMerge` avec le groupe font-size (+ ombres custom `shadow-card/float/modal/sheet/glow`), verrouillé par `tests/unit/cn.test.ts`. *(fait le 2026-07-11)*
- [x] V07b — Timeline desktop : la colonne des noms laisse voir les filets de jours et des artefacts au scroll (le dégradé de filets est peint sur toute la rangée, y compris sous la colonne, et les 2 px d'interligne laissent passer les barres). Filets peints sur la piste seulement, cellules de noms opaques pleine hauteur (h-9, plus d'interligne transparent), label de couloir en cellule opaque — la colonne est un bandeau continu ; typo rééquilibrée : noms `text-body` semi-gras 13 px, vols `text-data` mono 12, logements/activités `text-body` 13. *(fait le 2026-07-11)*
- [x] V07c — Mes voyages : les voyages passés reprennent le **même format de carte que les actifs** mais en grisé (photo désaturée — la couleur revient au survol —, textes éteints, bordure au lieu de l'ombre), dans la même grille 2 colonnes — la rangée pleine largeur était « bizarre ». `TripPastRow` supprimé. *(fait le 2026-07-11)*
- [x] V07d — Idées : **numéroter les idées géolocalisées et les relier à la carte** (clic sur le numéro → la carte zoome sur le lieu, pastilles numérotées assorties, numérotation dans l'ordre d'affichage trié/filtré). Layout desktop façon « Carte » : liste à gauche, carte sticky à droite — les actions des cartes d'idées gardent leur place, la carte reste visible en scrollant ; en mobile la carte reste au-dessus et le clic la ramène à l'écran. Sa question « même système que la Carte ? » : oui pour le principe liste+carte, mais liste large / carte en colonne (l'inverse de la page Carte) pour préserver la place des actions. *(fait le 2026-07-11)*
