...
> **Exceptions assumées (données ou hors périmètre)** : format des **dates** (à faire dans une passe `date-fns` locale dédiée) ; **titres du catalogue Valise** (deviennent des `checklist_items.title` en base → les traduire désynchroniserait le dédoublonnage entre membres) ; quelques messages de validation Zod au niveau module et contenus d'**emails** ; `EVENT_TYPE_LABELS` sur la page publique `/p/[token]` ; `global-error` (hors provider). Mécanique incrémentale en place (repli FR) : ces restes se complètent à la demande.
