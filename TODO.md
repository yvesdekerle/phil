...
**Anglais-first (fait le 2026-07-08, demande Yves)** : l'anglais devient la **source de vérité** et la langue par défaut. `en.ts` n'est plus annoté `PartialMessages` → `Messages = typeof en` (complet) ; `fr`/`es` sont les surcouches ; `translator`/`completeMessages` retombent sur `en` ; `defaultLocale = "en"` (une session sans cookie voit l'anglais). Tests i18n bascule référence → en. Migration `20260708140000_profiles_default_locale_en.sql` (défaut `locale='en'` + trigger `handle_new_user`) — **à appliquer par Yves** ; les profils existants gardent leur langue. Fix crash lié : couverture d'hôte externe → `<img>` (commit `6a8385e`). NB vérifié : en/es **déjà 100 % traduits** (1470/1470 clés ; les valeurs identiques au fr sont des noms propres/marques/cognats — rien à traduire).
