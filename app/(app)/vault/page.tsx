    ...
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8">
      ...
      {!coffreKey ? (
        <div className="mb-6 rounded-lg border border-line bg-card px-5 py-4">
          <p className="text-subhead text-ink">Sécurise ton coffre</p>
      ...
      <h2 className="mb-3 text-heading text-ink">Mes documents</h2>
      {documents.length === 0 ? (
            ...
            <li key={doc.id}>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-subhead text-ink">{doc.file_name}</span>
                  <span className="block font-mono text-label text-mist uppercase tabular-nums">
                ...
                {doc.expires_at ? (
                  parseISO(doc.expires_at) < new Date() ? (
                    <span className="shrink-0 rounded-sm bg-berry-wash px-2 py-0.5 font-mono text-label text-berry-ink uppercase">
                      {t("vault.expired")}
                    </span>
                  ) : parseISO(doc.expires_at) < addDays(new Date(), 90) ? (
                    <span className="shrink-0 rounded-sm bg-citron-wash px-2 py-0.5 font-mono text-label text-ink uppercase">
                      {t("vault.expiresSoon")}{" "}
            ...
            {sharedWithMe.map((s) => (
              <li key={s.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-subhead text-ink">{s.fileName}</span>
                    <span className="block font-mono text-label text-mist uppercase tabular-nums">
